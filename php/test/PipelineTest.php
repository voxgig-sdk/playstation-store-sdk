...
// PlaystationStore SDK pipeline test
//
...
{
    private static function utility(): PlaystationStoreUtility
    {
        return new PlaystationStoreUtility();
    }
    ...
    private static function ctx(array $over = []): PlaystationStoreContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new PlaystationStoreContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new PlaystationStoreOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof PlaystationStoreError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new PlaystationStoreOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = PlaystationStoreMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new PlaystationStoreOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = PlaystationStoreMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new PlaystationStoreOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = PlaystationStoreMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new PlaystationStoreSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = PlaystationStoreMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new PlaystationStoreResponse([]),
            'result' => new PlaystationStoreResult([]),
        ]);
        [, $err] = PlaystationStoreMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new PlaystationStoreSpec([]),
            'response' => null,
            'result' => new PlaystationStoreResult([]),
        ]);
        [, $err] = PlaystationStoreMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new PlaystationStoreSpec(['step' => 's']),
            'response' => new PlaystationStoreResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new PlaystationStoreResult([]),
        ]);
        [, $err] = PlaystationStoreMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new PlaystationStoreSpec(['step' => 's']),
            'response' => new PlaystationStoreResponse(self::resp(200, ['v' => 1])),
            'result' => new PlaystationStoreResult([]),
        ]);
        [, $err] = PlaystationStoreMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new PlaystationStoreSpec(['step' => 's']),
            'response' => new PlaystationStoreResponse(self::resp(200, ['v' => 2])),
            'result' => new PlaystationStoreResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        PlaystationStoreMakeResponse::call($ctx);
    ...
    {
        $preset = new PlaystationStoreResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new PlaystationStoreSpec([]),
            'response' => new PlaystationStoreResponse([]),
            'result' => new PlaystationStoreResult([]),
        ...
        [$got, $err] = PlaystationStoreMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new PlaystationStoreResult([])]);
        [, $err] = PlaystationStoreMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new PlaystationStoreSpec([]), 'result' => null]);
        [, $err] = PlaystationStoreMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new PlaystationStoreSpec(['step' => 's']),
            'result' => new PlaystationStoreResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new PlaystationStoreOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = PlaystationStoreMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new PlaystationStoreSpec(['step' => 's']),
            'result' => new PlaystationStoreResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new PlaystationStoreOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = PlaystationStoreMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new PlaystationStoreResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new PlaystationStoreSpec([]),
            'result' => new PlaystationStoreResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = PlaystationStoreMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = PlaystationStoreMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new PlaystationStoreError('boom', 'boom');
        $utility->fetcher = function (PlaystationStoreContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new PlaystationStoreSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = PlaystationStoreMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (PlaystationStoreContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = PlaystationStoreMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(PlaystationStoreResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        PlaystationStoreMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (PlaystationStoreContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new PlaystationStoreSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = PlaystationStoreMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new PlaystationStoreResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new PlaystationStoreSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = PlaystationStoreMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = PlaystationStoreMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = PlaystationStoreMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new PlaystationStoreResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, PlaystationStoreDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new PlaystationStoreResult(['ok' => false])]);
        $this->expectException(PlaystationStoreError::class);
        PlaystationStoreDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new PlaystationStoreResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', PlaystationStoreMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        PlaystationStoreMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new PlaystationStoreBaseFeature();
        $b = new PlaystationStoreBaseFeature();
        PlaystationStoreFeatureAdd::call($ctx, $a);
        PlaystationStoreFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): PlaystationStoreBaseFeature
    {
        $f = new PlaystationStoreBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        PlaystationStoreFeatureAdd::call($ctx, self::named_feature('a'));
        PlaystationStoreFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        PlaystationStoreFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        PlaystationStoreFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        PlaystationStoreFeatureAdd::call($ctx, $replace);
        ...
        PlaystationStoreFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): PlaystationStoreContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new PlaystationStoreSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = PlaystationStorePrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        PlaystationStorePrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new PlaystationStoreResponse(['headers' => null]),
            'result' => new PlaystationStoreResult([]),
        ]);
        PlaystationStoreResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        PlaystationStoreResultBody::call($ctx);
