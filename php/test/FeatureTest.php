...
// PlaystationStore SDK feature test
//
...
class FtCtrl extends PlaystationStoreControl
{
...
class FtClient
{
    ...
    private PlaystationStoreUtility $utility;

    public function __construct(PlaystationStoreUtility $utility, array $options)
    {
    ...
    public function get_utility(): PlaystationStoreUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (PlaystationStoreContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public PlaystationStoreContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (PlaystationStoreContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = PlaystationStoreConfig::make_feature($name);
    ...
    private static function buildUrl(PlaystationStoreSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof PlaystationStoreError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new PlaystationStoreResponse($fetched);
        }
    ...
    private function fail(PlaystationStoreContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (PlaystationStoreContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
