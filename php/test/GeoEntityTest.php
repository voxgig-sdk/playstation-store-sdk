...
class GeoEntityTest extends TestCase
{
    ...
    {
        $testsdk = PlaystationStoreSDK::test(null, null);
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set PLAYSTATION_STORE_TEST_GEO_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function geo_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["geo01", "geo02", "geo03"] as $k) {
    ...
    $entid_env_raw = getenv("PLAYSTATION_STORE_TEST_GEO_ENTID");
