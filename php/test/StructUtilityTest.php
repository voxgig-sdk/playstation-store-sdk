...
// PlaystationStore SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = PlaystationStoreOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(PlaystationStoreOmni::structfix($now, Struct::undef()))
                        !== json_encode(PlaystationStoreOmni::structfix($val, Struct::undef()))) {
