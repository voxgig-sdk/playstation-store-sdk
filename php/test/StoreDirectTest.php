...
class StoreDirectTest extends TestCase
{
    public function test_direct_list_store(): void
    {
        ...
        if ($setup["live"]) {
            foreach (["age01", "country01", "language01", "search_string01"] as $_liveKey) {
    ...
    public function test_direct_load_store(): void
    {
        $setup = store_direct_setup(["id" => "direct01"]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-load-store", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
