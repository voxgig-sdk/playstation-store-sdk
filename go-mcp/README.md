# playstation-store-mcp

[MCP](https://modelcontextprotocol.io) server exposing the PlaystationStore SDK as
two agent tools — `playstation-store_list` and `playstation-store_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/playstation-store-mcp)
make build
...
claude mcp add --scope user playstation-store \
  -- /absolute/path/to/playstation-store-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./playstation-store-mcp -transport http -addr :8080
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/playstation-store-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user playstation-store \
     -- "$PWD"/dist/*/playstation-store-mcp -transport stdio
   ```
...
### Call the `playstation-store_list` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `playstation-store_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `playstation-store_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
```sh
./playstation-store-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"playstation-store_load","arguments":{"entity":"geo","query":{"id":1}}}}'
```
