...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.PLAYSTATIONSTORE_TEST_LIVE ||
    'TRUE' === process.env.PLAYSTATIONSTORE_TEST_OVERRIDE
  ) {
  ...
  m.PLAYSTATIONSTORE_TEST_EXPLAIN = process.env.PLAYSTATIONSTORE_TEST_EXPLAIN || m.PLAYSTATIONSTORE_TEST_EXPLAIN
