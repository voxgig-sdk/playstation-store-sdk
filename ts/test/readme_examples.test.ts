...
//     (\`new PlaystationStoreSDK(...)\` / \`PlaystationStoreSDK.test(...)\` become
//     \`PlaystationStoreSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'PlaystationStoreSDK'
...
const TEST_SEED = {"entity":{"geo":{"test01":{"id":"test01"}},"image":{"test01":{"id":"test01"}},"store":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new PlaystationStoreSDK(...)\` and any
// \`PlaystationStoreSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+PlaystationStoreSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/PlaystationStoreSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
