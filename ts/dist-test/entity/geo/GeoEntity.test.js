...
(0, node_test_1.describe)('GeoEntity', async () => {
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        for (const op of ['load']) {
            if ((0, utility_1.maybeSkipControl)(t, 'entityOp', 'geo.' + op, live))
                return;
        }
        ...
        let geo_ref01_data = Object.values(setup.data.existing.geo)[0];
        // LOAD
        ...
        (0, node_assert_1.default)(null != geo_ref01_data_dt0);
    });
});
function basicSetup(extra) {
    ...
    let idmap = transform(['geo01', 'geo02', 'geo03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
    ...
    const idmapEnvVal = process.env['PLAYSTATION_STORE_TEST_GEO_ENTID'];
