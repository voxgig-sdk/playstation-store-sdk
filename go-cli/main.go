// Command PlaystationStore-cli is a CLI + REPL for the PlaystationStore SDK,
...
const prompt = "playstation-store"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "geo image store"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      PLAYSTATION_STORE_APIKEY (api key), PLAYSTATION_STORE_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: PLAYSTATION_STORE_APIKEY carries the API key and
	// PLAYSTATION_STORE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("PLAYSTATION_STORE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("PLAYSTATION_STORE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewPlaystationStoreSDK(opts)
