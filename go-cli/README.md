# playstation-store-cli

AQL-driven command-line client **and** interactive REPL for the PlaystationStore
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/playstation-store-cli)
make build

# 2. See usage (words, entities, env vars)
./playstation-store-cli --help

# 3. Provide credentials once, via the environment
export PLAYSTATION_STORE_APIKEY=sk_live_xxx

# 4. Each command line is ONE AQL expression, run against the API:
./playstation-store-cli load 1 geo            # {id:1} shorthand
./playstation-store-cli load '{id:1}' geo       # explicit match map

# 5. Override the API base URL for a single call
PLAYSTATION_STORE_BASE=https://api.example.com ./playstation-store-cli load 1 geo

# 6. No arguments -> interactive REPL
./playstation-store-cli
playstation-store> load 1 geo
playstation-store> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/playstation-store-cli
   ```
...
3. **Run a query.** Evaluate an AQL expression against the API (or run with no
   ...
   ```sh
   ./dist/*/playstation-store-cli load 1 geo
   ```
...
```sh
./playstation-store-cli load 1 geo          # scalar shorthand for {id:1}
./playstation-store-cli load '{id:1}' geo     # explicit match map
```
...
```sh
export PLAYSTATION_STORE_APIKEY=sk_live_xxx            # API key
export PLAYSTATION_STORE_BASE=https://api.example.com  # optional: override the API base URL
./playstation-store-cli load 1 geo
```
...
Run with no arguments to open a REPL (prompt `playstation-store>`). Each line is
evaluated as its own AQL expression:

```text
$ ./playstation-store-cli
playstation-store> load 1 geo
playstation-store> /help
playstation-store> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an AQL atom (e.g. `geo`).
...
| Variable | Purpose |
|----------|---------|
| `PLAYSTATION_STORE_APIKEY` | API key sent with every request. |
| `PLAYSTATION_STORE_BASE` | Optional override of the API base URL. |
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/playstation-store-cli`. |
...
(`./playstation-store-cli <expr>`) and interactively (the REPL), and expressions compose the
