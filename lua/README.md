# PlaystationStore Lua SDK

The Lua SDK for the PlaystationStore API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Geo(nil):load({ id = "example_id" }, nil)
...
local result, err = client:PlaystationStore(nil):load(
  { id = "test01" }, nil
)
...
```
lua/
├── playstation-store_sdk.lua    -- Main SDK module
...
The main module (`playstation-store_sdk`) exports the SDK constructor
