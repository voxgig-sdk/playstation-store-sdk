...
require("playstation-store_types")
...
local PlaystationStoreSDK = {}
PlaystationStoreSDK.__index = PlaystationStoreSDK
...
PlaystationStoreSDK._make_feature = _make_feature


function PlaystationStoreSDK.new(options)
  local self = setmetatable({}, PlaystationStoreSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function PlaystationStoreSDK:options_map()
...
function PlaystationStoreSDK:get_utility()
  return Utility.copy(self._utility)
end


function PlaystationStoreSDK:get_root_ctx()
  return self._rootctx
end


function PlaystationStoreSDK:prepare(fetchargs)
...
function PlaystationStoreSDK:direct(fetchargs)
...
function PlaystationStoreSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function PlaystationStoreSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "PlaystationStoreSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function PlaystationStoreSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function PlaystationStoreSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "PlaystationStoreSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function PlaystationStoreSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = PlaystationStoreSDK.new(sdkopts)
  sdk.mode = "test"
