...
local GeoEntity = {}
GeoEntity.__index = GeoEntity


function GeoEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GeoEntity)
  self._name = "geo"
  self._client = client
...
function GeoEntity:get_name()
  return self._name
end


function GeoEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GeoEntity.new(self._client, opts)
end
...
function GeoEntity:mark_deleted()
  self._deleted = true
end


function GeoEntity:deleted()
  return true == self._deleted
end


function GeoEntity:data_set(args)
  if args ~= nil then
...
function GeoEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GeoEntity:match_set(args)
  if args ~= nil then
...
function GeoEntity:stream(action, args, callopts)
...
function GeoEntity:_run_op(ctx, post_done)
  local utility = self._utility
