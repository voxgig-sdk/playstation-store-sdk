...
from playstationstore_sdk.core.control import PlaystationStoreControl
...
from playstationstore_sdk.core.helpers import get_ctx_prop, to_map
...
class PlaystationStoreContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = PlaystationStoreControl()
        ...
        if isinstance(sp, PlaystationStoreSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, PlaystationStoreResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, PlaystationStoreResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return PlaystationStoreOperation({})
    ...
    def make_error(self, code, msg):
        return PlaystationStoreError(code, msg, self)
