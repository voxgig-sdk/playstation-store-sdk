...
from playstationstore_sdk.core.spec import PlaystationStoreSpec
from playstationstore_sdk.core.result import PlaystationStoreResult
from playstationstore_sdk.core.response import PlaystationStoreResponse
from playstationstore_sdk.core.operation import PlaystationStoreOperation
from playstationstore_sdk.core.error import PlaystationStoreError
...
def _err_from_map(m):
    ...
    if msg == "":
        return None
    code = m.get("code", "")
    return PlaystationStoreError(code, msg)
...
class TestPrimaryUtility:
    ...
    def test_done_basic(self):
        ...
        def subject(entry):
            ...
            except PlaystationStoreError as e:
                return None, e
    ...
    def test_make_error_no_throw(self):
        ...
        ctx.result = PlaystationStoreResult({
            "ok": False,
            "resdata": {"id": "safe01"},
        })
    ...
    def test_feature_init_basic(self):
        init_client = PlaystationStoreSDK.test(None, None)
        ...
        class TestInitFeature(PlaystationStoreBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "initfeat"
                self.active = True
    ...
    def test_feature_init_inactive(self):
        ...
        class TestInitFeatureInactive(PlaystationStoreBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "nofeat"
                self.active = False
    ...
    def test_fetcher_live(self):
        ...
        live_client = PlaystationStoreSDK({
    ...
    def test_fetcher_blocked_test_mode(self):
        ...
        blocked_client = PlaystationStoreSDK({
            "base": "http://localhost:8080",
            "system": {
                "fetch": mock_fetch,
            },
    ...
    def test_make_result_no_result(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = PlaystationStoreSpec({"step": "start"})
        ctx.result = None
    ...
    def test_make_spec_basic(self):
        ...
        spec_client = PlaystationStoreSDK.test(None, setup_opts)
    ...
    def test_operator_basic(self):
        ...
        def subject(entry):
            ...
            if not isinstance(in_val, dict):
                in_val = {}
            op = PlaystationStoreOperation(in_val)
            return {
                "entity": op.entity,
                "name": op.name,
                "input": op.input,
    ...
    def test_prepare_auth_basic(self):
        ...
        auth_client = PlaystationStoreSDK.test(None, setup_opts)
