# PlaystationStore SDK test runner
...
class PlaystationStoreTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            PlaystationStoreTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = PlaystationStoreTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = PlaystationStoreTestRunner.getenv("PLAYSTATION_STORE_TEST_LIVE")
        override = PlaystationStoreTestRunner.getenv("PLAYSTATION_STORE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = PlaystationStoreTestRunner.getenv(key)
        ...
        explain = PlaystationStoreTestRunner.getenv("PLAYSTATION_STORE_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["PLAYSTATION_STORE_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if PlaystationStoreTestRunner._test_control is not None:
            return PlaystationStoreTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                PlaystationStoreTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            PlaystationStoreTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = PlaystationStoreTestRunner.load_test_control()
...
def load_env_local():
    PlaystationStoreTestRunner.load_env_local()


def env_override(m):
    return PlaystationStoreTestRunner.env_override(m)


def entity_data(v):
    return PlaystationStoreTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return PlaystationStoreTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return PlaystationStoreTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return PlaystationStoreTestRunner.load_test_control()


def live_client_options():
    return PlaystationStoreTestRunner.live_client_options()


def live_delay_ms():
    return PlaystationStoreTestRunner.live_delay_ms()
