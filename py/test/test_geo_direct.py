...
class TestGeoDirect:
    ...
    def test_should_direct_load_geo(self):
        setup = _geo_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-geo", "live" if setup["live"] else "unit")
        if _skip:
