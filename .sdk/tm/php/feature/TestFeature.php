...
// PlaystationStore SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class PlaystationStoreTestFeature extends PlaystationStoreBaseFeature
{
    ...
    public function init(PlaystationStoreContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (PlaystationStoreContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = PlaystationStoreParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (PlaystationStoreContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(PlaystationStoreContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = PlaystationStoreParam::call($ctx, $k);
