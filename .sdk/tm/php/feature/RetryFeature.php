...
// PlaystationStore SDK retry feature
...
class PlaystationStoreRetryFeature extends PlaystationStoreBaseFeature
{
    ...
    public function init(PlaystationStoreContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (PlaystationStoreContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(PlaystationStoreContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(PlaystationStoreContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
