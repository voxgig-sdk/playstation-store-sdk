...
module PlaystationStoreUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = PlaystationStoreResponse.new({})
    ...
    if fetch_err
      ...
      response = PlaystationStoreResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = PlaystationStoreResponse.new(fetched)
    else
