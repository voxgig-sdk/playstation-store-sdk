...
type PlaystationStoreSDK struct {
	Mode     string
...
func (sdk *PlaystationStoreSDK) OptionsMap() map[string]any {
...
func (sdk *PlaystationStoreSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *PlaystationStoreSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *PlaystationStoreSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *PlaystationStoreSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *PlaystationStoreSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *PlaystationStoreSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("PlaystationStoreSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *PlaystationStoreSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *PlaystationStoreSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("PlaystationStoreSDK: graphql: %s", msg)
		res["graphql"] = errors
	}
...
func TestSDK(testopts map[string]any, sdkopts map[string]any) *PlaystationStoreSDK {
	...
	sdk := NewPlaystationStoreSDK(sdkopts)
	sdk.Mode = "test"
