...
func TestImageDirect(t *testing.T) {
	t.Run("direct-load-image", func(t *testing.T) {
		...
		if setup.live {
			for _, _liveKey := range []string{"age01", "container_id01", "cusa01", "language01"} {
...
func imageDirectSetup(mockres any) *imageDirectSetupResult {
	...
	env := envOverride(map[string]any{
		"PLAYSTATION_STORE_TEST_IMAGE_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["PLAYSTATION_STORE_TEST_IMAGE_ENTID"]; ok {
