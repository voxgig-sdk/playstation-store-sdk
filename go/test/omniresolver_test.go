...
import (
	...
	omni "github.com/voxgig-sdk/playstation-store-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.PlaystationStoreError); is {
				out := map[string]any{
					"name":    "PlaystationStoreError",
...
func omniCtx(arg any, client *sdk.PlaystationStoreSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.PlaystationStoreSDK); is {
			client = live
