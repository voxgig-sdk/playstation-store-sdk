...
func TestGeoEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set PLAYSTATION_STORE_TEST_GEO_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		geoRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.geo", setup.data)))
		var geoRef01Data map[string]any
		if len(geoRef01DataRaw) > 0 {
			geoRef01Data = core.ToMapAny(geoRef01DataRaw[0][1])
		}
...
func geoBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("PLAYSTATION_STORE_TEST_GEO_ENTID")
	...
	idmapResolved := core.ToMapAny(env["PLAYSTATION_STORE_TEST_GEO_ENTID"])
