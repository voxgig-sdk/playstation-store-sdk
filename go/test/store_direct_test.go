...
func TestStoreDirect(t *testing.T) {
	t.Run("direct-load-store", func(t *testing.T) {
		...
		if setup.live {
			for _, _liveKey := range []string{"age01", "country01", "cusa01", "language01"} {
...
func storeDirectSetup(mockres any) *storeDirectSetupResult {
	...
	env := envOverride(map[string]any{
		"PLAYSTATION_STORE_TEST_STORE_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["PLAYSTATION_STORE_TEST_STORE_ENTID"]; ok {
