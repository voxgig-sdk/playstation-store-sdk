...
class StoreEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = PlaystationStoreSDK.test(seed, nil)
    seen = base.Store(nil).stream("list", nil, nil).to_a
    assert_equal 3, seen.length

    # Inbound: streaming active -> yields each item from the feature.
    cfg = PlaystationStoreConfig.make_config
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      sdk = PlaystationStoreSDK.test(seed, { "feature" => { "streaming" => { "active" => true } } })
      got = []
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set PLAYSTATION_STORE_TEST_STORE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    store_ref01_data_dt0_load_result = Helpers.to_map(store_ref01_data_dt0_loaded.respond_to?(:data_get) ? store_ref01_data_dt0_loaded.data_get : store_ref01_data_dt0_loaded)
...
def store_basic_setup(extra)
  ...
  entid_env_raw = ENV["PLAYSTATION_STORE_TEST_STORE_ENTID"]
