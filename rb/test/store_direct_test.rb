...
class StoreDirectTest < Minitest::Test
  def test_direct_list_store
    ...
    if setup[:live]
      ["age01", "country01", "language01", "search_string01"].each do |_live_key|
  ...
  def test_direct_load_store
    setup = store_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-store", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
