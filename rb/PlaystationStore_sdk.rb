...
require_relative 'PlaystationStore_types'
...
class PlaystationStoreSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = PlaystationStoreHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = PlaystationStoreHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, PlaystationStoreFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    PlaystationStoreUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = PlaystationStoreHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = PlaystationStoreHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = PlaystationStoreHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => PlaystationStoreError.new(
        "#{op}_allow",
        "PlaystationStoreSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue PlaystationStoreError => err
    ...
    if fetched.is_a?(Hash)
      status = PlaystationStoreHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = PlaystationStoreError.new(
        "graphql_error", "PlaystationStoreSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = PlaystationStoreSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
