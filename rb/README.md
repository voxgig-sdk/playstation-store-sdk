# PlaystationStore Ruby SDK

The Ruby SDK for the PlaystationStore API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
result, err = client.Geo(nil).load({ "id" => "example_id" }, nil)
raise err if err
puts result
```
...
result, err = client.PlaystationStore(nil).load(
  { "id" => "test01" }, nil
)
...
```
rb/
├── PlaystationStore_sdk.rb       -- Main SDK module
