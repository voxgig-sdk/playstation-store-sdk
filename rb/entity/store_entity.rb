...
class StoreEntity
  ...
  def data_set(args)
    if args
      @_data = PlaystationStoreHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = PlaystationStoreHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # Load a single Store.
  #
  # @param reqmatch [StoreLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Store.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Store, Hash] the loaded Store; raises PlaystationStoreError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = PlaystationStoreHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Store>, Array] the matching Store items; raises PlaystationStoreError on failure
